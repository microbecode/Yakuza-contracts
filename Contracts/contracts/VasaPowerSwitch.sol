...
contract VasaPowerSwitch {
    ...
    function getContextInfo(uint256 amount) public view returns (uint256 timeWindow2, uint256 multiplier, uint256 divider, uint256 mintable) {
        if(amount == 0 || block.number > _timeWindows[_timeWindows.length - 1]) {
            return (0, 0, 0, 0);
        }
        uint256 i = 0;
